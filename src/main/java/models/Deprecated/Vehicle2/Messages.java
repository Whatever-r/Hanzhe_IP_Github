package models.Deprecated.Vehicle2;

import simudyne.core.graph.Message;

public class Messages {
	public static class buyICE extends Message.Empty {
	}
	
	public static class sellICE extends Message.Empty {
	}
	
	public static class buyEV extends Message.Empty {
	}
	
	public static class vInfo extends Message {
		double evPrice;
		double evCost;
		double icePrice;
		double iceCost;
	}
//
//		public vInfo(double evPrice, double evCost, double icePrice, double iceCost) {
//			this.evPrice = evPrice;
//			this.evCost = evCost;
//			this.icePrice = icePrice;
//			this.iceCost = iceCost;
//		}

//	public static class evInfo extends Message {
//		double evPrice;
//		double evCost;
//	}
//
//	public static class iceInfo extends Message {
//		double icePrice;
//		double iceCost;
//	}
//
//	public static class Arrears extends Message {
//		int monthsInArrears;
//		int outstandingBalance;
//	}
//
//	public static class LoanDefault extends Message.Empty {
//	}
//
//	public static class CloseMortgageAmount extends Message.Integer {
//	}
//
//	public static class MortgageApplication extends Message {
//		int amount;
//		int income;
//		int wealth;
//	}
//
//	public static class ApplicationSuccessful extends Message {
//		int amount;
//		int termInMonths;
//		int repayment;
//	}
//
//	public static class Payment extends Message {
//		int repayment;
//		int amount;
//	}
}
